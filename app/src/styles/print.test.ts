...
describe("the print palette", () => {
  ...
  test("every colour that draws text reaches 4.5:1 on white", () => {
    // Text colours: the body, the quiet labels, the status colours, and every ANSI step the highlighter maps onto.
    const TEXT = new Set(["--ink", "--ink-2", "--accent", "--warn", "--danger", "--ok"]);
    const textTokens = [...palette.keys()].filter((name) => TEXT.has(name) || name.startsWith("--ansi-"));
    expect(textTokens.length).toBeGreaterThanOrEqual(15);
