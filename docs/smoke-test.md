...
- [x] Copying a mouse selection, or ⌘C over a selection, shows "copied to clipboard" at the top right of the pane
      for 1.5 s; an OSC 52 write shows nothing, because Herdr shows its own — `automated (selection.e2e.ts)`
- [ ] The toast looks right: gold border and dot on the dark ground, top right, gone after about 1.5 s, and a click
