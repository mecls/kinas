...
//
// `pushing` is a repository with a remote (tasks/tree-changes-push/prd.md): its branch, named with the token so the log
// grep can look for it, pushed with -u to a bare remote beside `root` — no tree lists the remote. `repo` keeps no remote,
// so a ↻ there still clears everything and the older steps keep their meaning.
...
export function fixture(dataDir: string): Record<string, string> {
  ...
  mkdirSync(join(pushing, "docs"), { recursive: true });
  writeFileSync(join(pushing, `README-${TOKEN}.md`), PUSHING_TEXT);
  writeFileSync(join(pushing, "docs", `plan-${TOKEN}.md`), `# Plan ${TOKEN}\n`);
  writeFileSync(join(pushing, ".gitignore"), "notes/\n");
  const remote = join(dataDir, `remote-${TOKEN}.git`);
  mkdirSync(remote);
  git(remote, "init", "-q", "--bare");
  git(pushing, "init", "-q", "-b", BRANCH);
  git(pushing, "add", "-A");
  git(pushing, "commit", "-q", "-m", "the pushing fixture");
  git(pushing, "remote", "add", "origin", remote);
  git(pushing, "push", "-q", "-u", "origin", BRANCH);
...
/** No global hook, signing or editor gets in the way of the fixture's commits, or the spec's commits and pushes. */
