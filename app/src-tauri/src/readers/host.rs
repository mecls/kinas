...
//! defined once: memory used = total − available; disk = the `/System/Volumes/Data` volume, used =
//! total − available to the user (purgeable space counts as used).
...
impl HostSampler {
    pub fn sample(&mut self) -> Result<HostReading, String> {
        ...
        let (disk_total, disk_available) = volume_space(DATA_VOLUME)?;
        Ok(HostReading {
            ...
            disk_used_gb: (disk_total - disk_available).max(0.0) / GIB,
...
pub fn write_host(conn: &Connection, org_id: &str, machine: &str, reading: &HostReading, now: i64) -> rusqlite::Result<()> {
    conn.execute(
        "INSERT INTO hosts (org_id, machine, cpu_pct, mem_used_gb, mem_total_gb, disk_used_gb, disk_total_gb, services, updated_at)
         ...
         ON CONFLICT (org_id, machine) DO UPDATE SET
           ...
           disk_used_gb = excluded.disk_used_gb, disk_total_gb = excluded.disk_total_gb, updated_at = excluded.updated_at",
        params![org_id, machine, reading.cpu_pct, reading.mem_used_gb, reading.mem_total_gb, reading.disk_used_gb, reading.disk_total_gb, now],
    )?;
    Ok(())
}
...
mod tests {
    ...
    fn writing_twice_keeps_one_row_per_machine() {
        ...
        let reading = HostReading { cpu_pct: 12.5, mem_used_gb: 7.0, mem_total_gb: 16.0, disk_used_gb: 200.0, disk_total_gb: 460.0 };
        write_host(&store.conn(), store.org_id(), "mac", &reading, 1).unwrap();
        write_host(&store.conn(), store.org_id(), "mac", &HostReading { cpu_pct: 50.0, ..reading }, 2).unwrap();
        let (count, cpu, updated): (i64, f64, i64) = store
            .conn()
            .query_row("SELECT count(*), max(cpu_pct), max(updated_at) FROM hosts", [], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)))
            .unwrap();
        assert_eq!((count, cpu, updated), (1, 50.0, 2));
    }
}
