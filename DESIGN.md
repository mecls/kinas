...
- **Status colors** appear as dots, bars, and segments. They are never text, with one exception: `--danger` for an error message (5.7:1 on `--surface`). (1.5, tree changes: and, in the Diff only, a faint tint behind a whole line — `--ok` for an added line, `--danger` for a removed one, mixed into `--surface` — always with the line's + or − beside it, so the sign says in a word what the tint says in colour. The contrast of `--ink` on both tints is measured into 2.4 when the Diff is built.)
